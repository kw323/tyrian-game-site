...
function getSaveBackupDirectory(): string {
...
function getSaveFilename(key: string): string | null {
  if (key === 'tyrian_autosave') return 'autosave.json';
    ...
    const server = http.createServer((request: any, response: any) => {
      ...
      if (requestUrl.pathname === '/api/save-backup' && request.method === 'POST') {
        ...
        request.on('data', (chunk: Buffer) => {
          if (chunks.reduce((size, item) => size + item.length, 0) < 4 * 1024 * 1024) chunks.push(chunk);
        });
        request.on('end', () => {
          try {
            const body = JSON.parse(Buffer.concat(chunks).toString('utf8')) as { key?: string; payload?: string };
