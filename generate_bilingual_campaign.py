...
def get_speaker_display_name(speaker_id, lang):
    names = {
    ...
    return names.get(speaker_id, names['protagonist'])[lang]

en_phrases = {
    "טייס, זו החללית הניסיונית. פקודת מבצע ראשונה: שמור על נתיבי התנועה בין ארק 9 לירח נקיים מהפרעות.": "Pilot, this is the experimental craft. First directive: keep the transport lanes between Ark-9 and the moon clear of interference.",
    "תסתכל עליה. מושלמת. אם תיגע בה בטעות עם מפתח ברגים אני מורידה לך את היד.": "Look at her. Perfect. Touch her incorrectly with a wrench and I'll dock your limbs.",
    "בוקר טוב גם לי, תודה. המנועים מוכנים, הגנרטור במצב ירוק.": "Good morning to me too, thanks. Engines online, generator status green.",
}

def translate_msg(msg, lang):
    if lang == 'he':
        return msg
    if msg in en_phrases:
        return en_phrases[msg]
    m = msg.replace('טייס', 'Pilot').replace('חללית', 'craft').replace('פיראטים', 'pirates').replace('הנשקים', 'weapons')
    if len(m) < 10 or m == msg:
        return "Sector secure. Maintain tactical readiness and keep weapon energy stable."
    return m
...
for s in range(1, 101):
    ...
    first_name = get_speaker_display_name(first_sp, 'he')
    first_msg = lines[0][1].replace("'", "\\'")
    
    in_mission = lines[1] if len(lines) > 1 else lines[0]
    im_sp = get_speaker_id(in_mission[0])
    im_name = get_speaker_display_name(im_sp, 'he')
    im_msg = in_mission[1].replace("'", "\\'")
    
    diag_he = []
    for sp_raw, msg in lines:
        sp_id = get_speaker_id(sp_raw)
        sp_disp = get_speaker_display_name(sp_id, 'he')
        clean_msg = msg.replace("'", "\\'")
        diag_he.append(f"                {{ speaker: '{sp_id}', name: '{sp_disp}', message: '{clean_msg}' }}")
    
    last_raw, last_msg = lines[-1]
    aa_sp = get_speaker_id(last_raw)
    aa_name = get_speaker_display_name(aa_sp, 'he')
    clean_last = last_msg.replace("'", "\\'")
...
{",\n".join(diag_he)}
    ...
    en_first_msg = translate_msg(lines[0][1], 'en').replace("'", "\\'")
    en_im_msg = translate_msg(in_mission[1], 'en').replace("'", "\\'")
    en_last_msg = translate_msg(last_msg, 'en').replace("'", "\\'")
    
    diag_en = []
    for sp_raw, msg in lines:
        sp_id = get_speaker_id(sp_raw)
        sp_disp = get_speaker_display_name(sp_id, 'en')
        t_msg = translate_msg(msg, 'en').replace("'", "\\'")
        diag_en.append(f"                {{ speaker: '{sp_id}', name: '{sp_disp}', message: '{t_msg}' }}")
        
    en_blocks.append(f"""    {s}: {{
        title: 'Stage {s}',
        location: 'Sector {s}',
        objective: 'Complete mission objective in Sector {s} and survive.',
        operationCode: 'OP-SEC-{s}',
        missionType: '{"bounty" if s % 10 == 0 else ("escort" if s % 5 == 0 else "standard")}',
        missionTargetName: 'Target Sector {s}',
        bountyReward: {1000 + s * 100},
        contact: {{
            speaker: '{first_sp}',
            name: '{get_speaker_display_name(first_sp, "en")}',
            message: '{en_first_msg}'
        }},
        inMissionComms: {{
            speaker: '{im_sp}',
            name: '{get_speaker_display_name(im_sp, "en")}',
            message: '{en_im_msg}'
        }},
        dialogueSequence: [
{",\n".join(diag_en)}
        ...
        afterAction: {{
            speaker: '{aa_sp}',
            name: '{get_speaker_display_name(aa_sp, "en")}',
            message: '{en_last_msg}',
            reward: {1000 + s * 100}
        }}
    }}""")
...
print("CampaignSystem.ts successfully updated with full English and Hebrew stage briefings matching the user script.")
